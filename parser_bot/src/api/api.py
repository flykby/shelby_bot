...
class API:
    ...
    def __init__(self):
        ...
        self.user_token = conf.USER_TOKEN
        self.api_url = conf.API_URL
        self.my_steam_id = conf.MY_STEAM_ID
    ...
    def update_order(self, order: dict) -> bool:
        ...
        try:
            create_res = requests.patch(f"{self.api_url}/v2/classifieds/listings/{order['id']}?token={self.user_token}", headers=headers, json=params)
    ...
    def get_stat_order(self, order: dict) -> list:
        orders = requests.get(url=f"{self.api_url}/classifieds/listings/snapshot?token={self.user_token}&sku={order['item']['name']}&appid=440")
        orders = orders.json()
        print("-----STAT ORDER-----")
        print(orders)
        result = []
